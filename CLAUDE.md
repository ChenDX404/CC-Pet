...
- 桌宠渲染唯一路径：**Tauri 桌面浮窗**（`dist-pet/pet.js`）
- VS Code Extension 仅作为 launcher + 状态桥 + GUI 绑定编辑器
- 动画架构：纯 JS setInterval（乒乓循环 / 正向播放），无 CSS animation，`dist-pet/pet.js` 中的 `startLoop()` / `playOnce()` / `stopAnimation()` / `frame()` 四个函数驱动全部动画
...
| **C 离线行为**（Claude Code 未运行 / 未安装） | 显示**闲置动画**（idle 帧），不报错、不隐藏 | 阶段 4 | ❌ 阶段 1 无状态检测 |
...
### 用户配置文件：`src/pet-renderer/pet-config.json`

**这是用户唯一需要手动编辑的配置文件**。包含所有帧数据和事件绑定。

### 文件位置
```
项目根目录 / pet-config.json
```

### 用户需要填的字段

配置文件只有 `row` + `frames`，**没有 name**。每行叫什么名由 CC Pet 内置约定俗成表决定（行 1=idle, 行 2=running-left, ...）。GUI 阶段用户可以自定义覆盖。

| 字段 | 含义 | 当前值 |
|---|---|---|
| `rows[].row` | 行号（1-11，顺序固定） | 1-11 |
| **`rows[].frames`** | **该行实际帧数（用户手动填！）** | 全部 8（**必须按实际 spritesheet 逐行修正**） |
| `bindings` | 事件 → 动作名绑定 | drag-left→running-right / drag-right→running-left / click→look-down |
| `colWidth` / `rowHeight` | 每格尺寸 | 192 / 208 |

### 怎么知道每行实际帧数

在 DevTools Console 用 `show(row, col)` 逐列试，看到空白的那一列就是最后一帧的下一列：

```javascript
const p = document.getElementById("pet");
function show(row, col) {
  p.style.backgroundPosition = '-' + (col*192) + 'px -' + ((row-1)*208) + 'px';
  p.style.width = '192px'; p.style.height = '208px';
}
show(8, 7)  // 第 8 行第 7 列（0-based）
```

如果 `show(8, 7)` 是空的但 `show(8, 6)` 不是 → review 行有 7 帧 → 改 `actions[8].frames = 7`

### 生效方式

修改 JSON 后 **Shift+F5 → F5** 重启调试即可生效。**不需要 `npm run compile`**。

### 注意事项

- ❌ 不要增删 `actions[]` 数组（必须是 11 项）
- ❌ 不要改 `actions[].name`（这是约定俗成名，代码依赖它）
- ✅ 只改 `actions[].frames` 的数字
- ✅ 可以改 `bindings` 把事件绑到不同动作
...
Claude Code hooks / 状态检测、16 方向精灵切换、动画状态机、Tauri（阶段 6 才引入）、Electron、桌面透明窗口、置顶、系统托盘、HTTP / WebSocket / MCP / 云端服务、登录、Marketplace 发布配置、自动 git 提交、GUI 绑定编辑器（阶段 5）。
...
**不要把范围外功能偷渡进来：**
24. ❌ 不要做 Claude Code hooks / 状态检测的任何探针
